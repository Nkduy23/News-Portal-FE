...
export default function DatNuocVaoXuanSection({ articles }: DatNuocVaoXuanSectionProps) {
  // Unique layout:
  // Left col: 1 large image card (with title overlaid)
  // Right col: 2 stacked cards each with small image + title + excerpt
  ...
  return (
    <section className="mt-8">
      ...
      <div className="rounded-sm overflow-hidden p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <Link href={`/article/${main.slug}`} className="group relative block overflow-hidden rounded-sm">
              <div className="relative" style={{ height: 280 }}>
                ...
                <div className="absolute bottom-0 p-4">
                  <h3 className="text-[15px] font-bold text-white leading-snug line-clamp-3 group-hover:text-[var(--color-accent)] transition-colors">{main.title}</h3>
                  {main.excerpt && <p className="text-[12px] text-white/70 mt-1 line-clamp-2">{main.excerpt}</p>}
                </div>
              </div>
            </Link>
          )}

          {/* 2 stacked right: each has small image left + text right */}
            ...
            {stacked.map((article) => (
              <Link key={article.id} href={`/article/${article.slug}`} className="group flex gap-3 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-[13px] font-semibold leading-snug line-clamp-2 group-hover:text-[var(--color-accent)] transition-colors" style={{ color: "rgba(255,255,255,0.92)" }}>
                    {article.title}
                  </h3>
                  {article.excerpt && (
        ...
        {bottom.length > 0 && (
          <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${bottom.length}, 1fr)` }}>
            {bottom.map((article) => (
